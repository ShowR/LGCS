...
% Carga de parâmetros
<<echo=FALSE, warning=FALSE, results='hide', message=FALSE, highlight=FALSE, error=FALSE, fig.show='hide'>>=
# Ajusta diretorio de trabalho

source('inicializa.R')
inicializa()
setwd('J://Projetos//lgcs');
...
\local{Av. Osvaldo Aranha, 99 â 5Âº andar, CEP: 90035-190, Porto Alegre/RS}
...
% --- 
% EspaÃ§amentos entre linhas e parágrafos 
% --- 

% O tamanho do parágrafo é dado por:
\setlength{\parindent}{1.3cm}

% Controle do espaÃ§amento entre um parágrafo e outro:
\setlength{\parskip}{0.2cm}  % tente também \onelineskip

% EspaÃ§amento simples
\SingleSpacing
...
Estudos sobre a aplicação da teoria dos jogos cooperativos no GCS versam como principal questão o gerenciamento harmonioso das decisões entre os elos da cadeia \cite{Dobos.2010a}. O pressuposto está na existência de uma estrutura comum entre agentes na qual o ganho ou custo seja compartilhado, seguindo critérios de justiÃ§a definidos por axiomas \cite{Bezerra.2009}. \citeonline{Brink.2002} caracteriza a propriedade de justiÃ§a e eficiência pelo modelo de jogo cooperativo \emph{Shapley Value}. 
...
O presente artigo é uma abordagem sobre a execução do modelo \emph{Shapley Value} para equacionar a contribuição marginal entre agentes de uma CS. A precípua está no rateio do custo rodoviário itinerário --- comensurado em Km. Considerando uma hipotética coalizão formada por três fornecedores, adiciona-se indivíduos, um de cada vez, dando a cada um uma contribuição marginal $v(S \cup \{i\})$ --- $v(S)$ para $S$ indivíduos. À medida que os custos são ponderados atinge-se uma noção de justiÃ§a. A \autoref{sec:caso} estrutura a narrativa do caso de estudo. Na \autoref{sec:teoria} estuda-se os conceitos da teoria, exemplificando com o cenário fictício. A \autoref{sec:R} implementa o algoritmo na linguagem R\footnote{\citetext{R.2016}} --- viabilizando futuras simulações. Ademais, a \autoref{sec:conclusao} retoma a proposta adicionando um parecer de conclusão; e, enfim, a \autoref{sec:futuro} projetar novas lacunas com enlace para investigações em trabalhos futuros.
...
<<echo=FALSE, warning=FALSE, results='markup', message=FALSE, highlight=TRUE, error=FALSE, fig.align='center', fig.show='hold', fig.pos='H', fig.cap='Custo dos itinerários'>>=

coordUFRGS <- geocode('Porto Alegre')
coordUFMS <- geocode('Belo Horizonte')
coordUFRJ <-  geocode('Rio de Janeiro')
coordAsuncion <-  geocode('Asuncion')
coordUFPR <- geocode('Curitiba')

df <- data.frame(longitude = c(coordUFPR[,1],
                               coordUFRGS[,1],
                               coordUFPR[,1],
                               coordUFRJ[,1],
                               coordUFPR[,1],
...
ggmap(get_googlemap('Parana', 
                     markers = df, 
                     path = df, 
                     zoom = 5, 
                     maptype = c('roadmap'),
                     language = 'pt-BR')) + theme_gray() +
      ...
      annotate('text',
               ...
               color = 'Red',
               size = 3.5) +
      geom_point(aes(x = coordUFMS[,1], 
                     y = coordUFMS[,2], 
                 colour = "Origem - UFMS"), 
                 alpha = 0.3, 
                 size = 2,
                 position = "jitter") +
      geom_point(aes(x = coordUFPR[,1], 
                     y = coordUFPR[,2], 
                 colour = "Origem - UFPR"), 
                 alpha = 0.3, 
                 size = 1.4,
                 position = "jitter") +
      geom_point(aes(x = coordAsuncion[,1], 
                     y = coordAsuncion[,2], 
                 colour = "Origem - Assuncion"), 
                 alpha = 0.3, 
                 size = 2,
                 position = "jitter") +
      geom_point(aes(x = coordUFRJ[,1], 
                     y = coordUFRJ[,2], 
                 colour = "Origem - UFRJ"), 
                 alpha = 0.3, 
                 size = 2,
                 position = "jitter") +
      geom_point(aes(x = coordUFRGS[,1], 
                     y = coordUFRGS[,2], 
                 colour = "Destino - UFRGS"), 
                 alpha = 0.3, 
                 size = 4,
                 position = "jitter") +
...
<<echo=TRUE, warning=FALSE, results='markup', message=FALSE, highlight=FALSE, error=FALSE, fig.show='hide', background='gray95'>>=
...
<<echo=FALSE, warning=FALSE, results='hide', message=FALSE, highlight=FALSE, error=FALSE, fig.show='hold', fig.align='center', fig.cap='Cálculo do Shapley Value', fig.pos='H', fig.height=3, fig.width=6>>=

# Exibe grafico com os resultados
ggplot(data = shapleyValue, 
       aes(x = nomesAgentes, 
           y = shapleyValue[,1], 
           fill = nomesAgentes)) +
       ...
       theme(legend.position='none', 
             ...
             axis.line = element_line(size = 0.3, 
                                      colour = 'black')) +
       labs(title = 'Resultado do cálculo do Shapley Value', 
            x = 'Agentes', 
            y = 'Shapley Value') +
       scale_y_continuous(expand = c(0, 0), labels = waiver()) + 
